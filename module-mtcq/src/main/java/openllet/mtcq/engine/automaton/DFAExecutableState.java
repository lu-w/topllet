...
/**
 * Central class in the satisfiability check for MTCQs. It represents triples of a DFA state, time point, and
...
public class DFAExecutableState
{
    ...
    DFAExecutableState(DFA dfa, MetricTemporalConjunctiveQuery mtcq, int dfaState, int timePoint,
                       EdgeConstraintChecker edgeChecker)
    {
        this(dfa, mtcq, dfaState, null, null, timePoint, edgeChecker, false, null);
    }

    DFAExecutableState(DFA dfa, MetricTemporalConjunctiveQuery mtcq, int dfaState, int timePoint,
                       EdgeConstraintChecker edgeChecker, Timer timer)
    {
        this(dfa, mtcq, dfaState, null, null, timePoint, edgeChecker, false, timer);
    }

    DFAExecutableState(DFA dfa, MetricTemporalConjunctiveQuery mtcq, int dfaState, int timePoint,
                       EdgeConstraintChecker edgeChecker, boolean isInitial)
    {
        this(dfa, mtcq, dfaState, null, null, timePoint, edgeChecker, isInitial, null);
    }

    DFAExecutableState(DFA dfa, MetricTemporalConjunctiveQuery mtcq, int dfaState, int timePoint,
                       EdgeConstraintChecker edgeChecker, boolean isInitial, Timer timer)
    {
        this(dfa, mtcq, dfaState, null, null, timePoint, edgeChecker, isInitial, timer);
    }

    DFAExecutableState(DFA dfa, MetricTemporalConjunctiveQuery mtcq, int dfaState, QueryResult satBindings,
                       QueryResult unsatBindings, int timePoint, EdgeConstraintChecker edgeChecker)
    {
        this(dfa, mtcq, dfaState, satBindings, unsatBindings, timePoint, edgeChecker, false, null);
    }

    /**
     * Constructs a new executable state.
     *
     * @param dfa The DFA which corresponds to the given MTCQ.
     * @param mtcq The MTCQ for which the satisfiability check is performed.
                       ...
                       Timer timer)
    {
        ...
        _maxTimePoint = mtcq.getTemporalKB().size();
        _timer = timer;
        if (_timer != null)
            _timer.stop();
    ...
    public Collection<DFAExecutableState> execute()
    {
        ...
        if (!_isInitial && _satBindings != null)
            ...
            restrictSatToBindings = new QueryResultImpl(_mtcq);

        // Sinks are handled efficiently by just propagating all information directly.
        if (isInSink())
        {
            DFAExecutableState newState = new DFAExecutableState(_dfa, _mtcq, _dfaState, _timePoint + 1,
                    _edgeChecker, _timer);
        ...
        else if (canExecute())
        {
            ...
            for (Edge edge : edges)
            {
                ...
                DFAExecutableState newState = new DFAExecutableState(_dfa, _mtcq, edge.getToState(), _timePoint + 1,
                        _edgeChecker, _timer);
                            ...
                            for (BCQQuery bcq : edge.getBCQs())
                            {
                                ...
                                for (DFAExecutableState newState : newExecutableStates)
                                    if (edge.getToState() == newState._dfaState)
                                    {
                                        ...
                                        {
                                            QueryResult bindingResult = new QueryResultImpl(_mtcq);
